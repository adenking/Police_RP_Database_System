...
if ($tempCrimes !== null)
{
	...
	{
		if (in_array($tempCrime, $_POST['removeCrime']))
		{
			unset($tempCrimes[$i]);
		}
	}
...
$saved = mysqli_query($con, "UPDATE Licensedata SET Crimes='" . json_encode($tempCrimes) . "' WHERE UserName='" . $_POST['Username'] . "'");
 
 if($saved)
 {
	echo "Selected Crimes Cleared From " . $_POST['Username'];
 }

logIt($_POST['Username'], $prevData, $_SESSION['user']['username'], json_encode($tempCrimes), 1, $con);
