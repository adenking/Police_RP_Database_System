...
$currentCrimes = json_decode($row['Crimes'], true);
$prevData = $row['Crimes'];
$compiled = $_POST['Crime'] . "_" . $_POST['time'];
if ($currentCrimes === null)
{
	$currentCrimes = array($compiled);
}
else
{
	array_push($currentCrimes, $compiled);
}
$newCrimes = json_encode($currentCrimes);
$saved = mysqli_query($con, "UPDATE Licensedata SET Crimes='" . $newCrimes . "' WHERE UserName='" . $_POST['Username'] . "'");
 
 if($saved)
 {
	echo "Crime " . $_POST['Crime'] . " added to " . $_POST['Username'];
	logIt($_POST['Username'], $prevData, $_SESSION['user']['username'], $newCrimes, 1, $con);
 }
mysqli_close($con);
?>
